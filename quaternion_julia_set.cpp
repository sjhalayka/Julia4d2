...
int quaternion_julia_set::SaveOBJ(const HWND &progressbar, const HWND &status_text, const HANDLE &cancel_event)
{
	...
	if(!cancelled)
	{
		...
		vector<mc_xyz> vertex_with_index;

		for(set<mc_xyz>::iterator i = final_vertices.begin(); i != final_vertices.end(); i++)
		{
			vertex_with_index.push_back(*i);
			vertex_with_index[vertex_with_index.size() - 1].index = index++;
		}

		final_vertices.clear();

		for(size_t i = 0; i < vertex_with_index.size(); i++)
			final_vertices.insert(vertex_with_index[i]);
